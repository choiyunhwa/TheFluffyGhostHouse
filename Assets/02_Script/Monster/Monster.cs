...
public class Monster : MonoBehaviour
{
    ...
    {
        NavMeshHit hit;
        NavMesh.SamplePosition(transform.position + (Random.onUnitSphere * Random.Range(minWanderDistance, maxWanderDistance)), out hit, maxWanderDistance, NavMesh.AllAreas); //최단 거리
    ...
    private void AttacingUpdate()
    {
        if (playerDistance < attackDistance && IsPlayerInFieldOfView())
        {
            ...
            if (Time.time - lastAttackTime > attackRate)
            {
                ...
                //PlayerManager.Instance.Player.controller.GetComponent<IDamagaIble>().TakePhysicalDamage(damage);
                animator.speed = 1;
